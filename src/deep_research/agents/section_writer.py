...
2. **Attach citations** at claim granularity — cite the evidence source after each claim
...
async def section_writer(
    ...
    Returns:
        Dict with status, content, blocked_reason, missing_claims, citations_used.
    """
    ...
    claims_text = "\n\n".join(
        f"[Claim {cid}] Confidence: {c.get('confidence', 0):.2f} | "
